...
@IBDesignable
open class MKTextField : UITextField, UITextFieldDelegate  {
...
@objc public protocol MKTextFieldDelegate : NSObjectProtocol {
    
    @objc @available(iOS 2.0, *)
    optional func textFieldShouldBeginEditing(_ textField: MKTextField) -> Bool // return NO to disallow editing.
    @objc @available(iOS 2.0, *)
    optional func textFieldDidBeginEditing(_ textField: MKTextField) // became first responder
    @objc @available(iOS 2.0, *)
    optional func textFieldShouldEndEditing(_ textField: MKTextField) -> Bool // return YES to allow editing to stop and to resign first responder status. NO to disallow the editing session to end
    @objc @available(iOS 2.0, *)
    optional func textFieldDidEndEditing(_ textField: MKTextField) // may be called if forced even if shouldEndEditing returns NO (e.g. view removed from window) or endEditing:YES called
    
    @objc @available(iOS 2.0, *)
    optional func textField(_ textField: MKTextField, shouldChangeCharactersInRange range: NSRange, replacementString string: String) -> Bool // return NO to not change text
    
    @objc @available(iOS 2.0, *)
    optional func textFieldShouldClear(_ textField: MKTextField) -> Bool // called when clear button pressed. return NO to ignore (no notifications)
    @objc @available(iOS 2.0, *)
    optional func textFieldShouldReturn(_ textField: MKTextField) -> Bool // called when 'return' key pressed. return NO to ignore.
}
